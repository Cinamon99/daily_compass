...
class AppState extends ChangeNotifier {
  ...
  List<MemoItem> get memos => List.unmodifiable(_memos);
  ...
  Future<void> removeMemo(String id) async {
    _memos = _memos.where((e) => e.id != id).toList();
    await _storage.saveMemos(_memos);
    notifyListeners();
  }
  ...
  Future<void> clearCompletedMemos(MemoCategory category) async {
    _memos = _memos
        .where((e) => !(e.category == category && e.done))
        .toList();
    await _storage.saveMemos(_memos);
    notifyListeners();
  }
  ...
  List<MemoItem> memosOf(MemoCategory category) =>
      _memos.where((e) => e.category == category).toList();

  /// 今天排了期的备忘
  List<MemoItem> get todayMemos {
    final today = todayKey();
    return _memos.where((e) => e.planDate == today).toList();
  }
  ...
  Future<void> removeReminder(String id) async {
    final target = _reminders.firstWhere(
      (e) => e.id == id,
      orElse: () => ReminderItem(notifyId: 0, title: '', hour: 0),
    );
    _reminders = _reminders.where((e) => e.id != id).toList();
    await _storage.saveReminders(_reminders);
    notifyListeners();
    // 取消它占用的所有通知 ID
  ...
  List<ReminderItem> get routineReminders =>
      _reminders.where((e) => e.isRoutine).toList();
  ...
  Future<void> removeReview(String dateKey) async {
    _reviews = _reviews.where((e) => e.dateKey != dateKey).toList();
    await _storage.saveReviews(_reviews);
    notifyListeners();
  }
  ...
  double get averageScore {
    ...
    final total = _reviews.fold<int>(0, (sum, e) => sum + e.score);
